...
import { initiatePayment, bookOnlineEvent } from '@/app/apis';
...
export default function OnlineEventCheckout({ event }: { event: OnlineEventFullDetail }) {
	const { user } = useUser();

	const [ticketID, setTicketID] = useState('');
	const [loading, setLoading] = useState(false);
	
	const [purchaseForm, setPurchaseForm] = useState<OnlinePurchaseForm>({
		...defaultPurchaseForm,
	...
	const handleConfirmBook = () => {
		...
		bookOnlineEvent(event.id, purchaseForm)
			.then((res) => {
	...
	const handlePayment = async () => {
		try {
			...
			const totalAmount = (event.price) + (event.tax || 0) + (event.productFee || 0);
	...
	const totalAmount = event.price ? (event.price + event.productFee + event.tax) : event.price 
	return (
		<div className="event-dt-block p-80">
			<div className="container">
				{ticketID ? <BookingConfirmed event={event} tickets={purchaseForm.tickets} ticketID={ticketID} totalAmount={totalAmount}/> : (
					<div className="row">
											...
											<>
												<div className="order-total-block">
													...
													<div className="order-total-dt">
														...
														<div className="order-number">{event.tax}</div>
													</div>
													<div className="order-total-dt">
														<div className="order-text">Product Fee</div>
														<div className="order-number">{event.productFee}</div>
													</div>
													<div className="divider-line"></div>
													<div className="order-total-dt">
														<div className="order-text">Total</div>
														<div className="order-number">{`INR ${totalAmount}`}</div>
													</div>
												</div>
